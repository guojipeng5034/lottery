...
const { addPrize, resetDefault, delAll, delItem, prizeList, currentPrize, selectedPrize, submitData, changePrizePerson, changePrizeStatus, selectPrize, localImageList, deleteDialogVisible, prizeToDelete, confirmDelete, cancelDelete, hasWinnersForPrize } = usePrizeConfig()
const { t } = useI18n()

const deleteWarningText = computed(() => {
    if (prizeToDelete.value && hasWinnersForPrize(prizeToDelete.value.id)) {
        return t('dialog.dialogDelPrizeWarning')
    }
    return ''
})
</script>
...
<template>
  <div>
      ...
      >
        ...
        <div class="w-full max-w-xs form-control">
          ...
          <div class="flex gap-2" @click.stop>
            <button class="btn btn-error btn-xs" @click.stop="delItem(item, $event)">{{ t('button.delete') }}</button>
          </div>
        </div>
      </div>
    </VueDraggable>
    ...
    <Dialog
      ...
      :submit-func="confirmDelete"
      :cancel-func="cancelDelete"
    >
      <template #content>
        <div v-if="deleteWarningText" role="alert" class="w-full my-4 alert alert-warning">
          <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 stroke-current shrink-0" fill="none" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
          </svg>
          <span>{{ deleteWarningText }}</span>
        </div>
      </template>
    </Dialog>
  </div>
